...
This package provides access to the datasets published by the Reserve Bank of New Zealand (RBNZ). The statistics page of their website is https://www.rbnz.govt.nz/statistics. A summary of the available data can be found at the end of this document. On the RBNZ webite the data is made available in .xlsx and .xls files; this package will download those files and read them into R.

The author has no affiliation with the RBNZ. The copyright for the data can be found at https://www.rbnz.govt.nz/copyright.

## Usage

The function for downloading the data is `getSeries`. This takes as its first argument the name of the series and will return a list containing the fields "meta" and "data" which are data frames containing the metadata and actual data, respectively.
...
Each series has a help file so typing `?B1` and such like will tell you what options, if any, are available.
